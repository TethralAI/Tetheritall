export class OrmDeviceStore {
    ...
    async list(filter) {
        ...
        if (filter?.status)
            qb.andWhere('d.status = :s', { s: filter.status });
        if (filter?.capability)
            qb.andWhere(`d.capabilities::jsonb ? :cap`, { cap: filter.capability });
        const rows = await qb.getMany();
        return rows.map((r) => ({ id: r.id, capabilities: r.capabilities, status: r.status, createdAt: r.createdAt.getTime() }));
    }
}
export class OrmShadowStore {
    repo;
    constructor(repo) {
        this.repo = repo;
    }
    ...
    async applyUpdate(deviceId, version, patch) {
        ...
        if (existing && version <= existing.version)
            return { version: existing.version, reported: existing.reported, updatedAt: existing.updatedAt.getTime() };
...
//# sourceMappingURL=typeorm.stores.js.map