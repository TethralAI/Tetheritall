...
let EgressGuard = class EgressGuard {
    ...
    localOnly;
    repos;
    constructor(cache, classifier, minimizer, bus, localOnly, repos) {
        this.cache = cache;
    ...
    async evaluate(event) {
        ...
        if (this.localOnly.isEnabled()) {
            ...
            if (this.repos.ormEnabled)
                await this.repos.privacyRepo.save(this.repos.privacyRepo.create({ deviceId: event.deviceId, allowed: false, policyVersion: consent.policyVersion, reason: 'local_only_mode' }));
        ...
        if (!consent.allowed) {
            ...
            if (this.repos.ormEnabled)
                await this.repos.privacyRepo.save(this.repos.privacyRepo.create({ deviceId: event.deviceId, allowed: false, policyVersion: consent.policyVersion, reason: consent.reason ?? 'denied' }));
        ...
        if (this.repos.ormEnabled)
            await this.repos.privacyRepo.save(this.repos.privacyRepo.create({ deviceId: event.deviceId, allowed: true, policyVersion: consent.policyVersion, reason: null }));
